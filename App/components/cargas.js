		...
		<View style={styles.item}>
			<View style={styles.item1}>
				<View style={{paddingTop: 15}}>
					<Image
						source={{uri: foto}}
						style={{height: 55, width: 60, borderRadius: 40, borderWidth: 4, borderColor: '#FFf'}}
					/>
					<Text style={styles.detalle}>{nombreCliente}</Text>
					<View style={{flexDirection: 'row'}}>
						<Image source={Star} style={{marginStart: 8}} />
						<Text style={{marginStart: 5, color: '#fff', fontWeight: 'bold'}}>{calificacion}</Text>
					</View>
				</View>
...
export default class Cargas extends Component{
    ...
    async componentDidMount() {
		await fetch('http://192.168.0.143:3000/pedidos').then(res => res.json()).then(
			result => {
